"""
Заглушка для стадії ML-регресії.

Працює з уже підготовленими датасетами у `data/ml_datasets/regression`.
...
def run_regression_modeling(data_path: str = "data") -> None:
    """
    Заглушка для регресійного ML-пайплайну.

    На даному етапі:
    - перевіряє наявність `data/ml_datasets/regression`
    ...
    if not os.path.exists(base_path):
        print(
            "⚠️  Регресійний датасет не знайдено. "
    ...
    if df_train is None:
        print(
            "⚠️  Не вдалося знайти train-спліт для регресійного датасету. "
            "Очікуються файли train.parquet або train.csv."
        )
        return

    print("\n" + "=" * 60)
    print("🧪 ML РЕГРЕСІЯ (заглушка)")
    print("=" * 60)
    print(f"Форма train-датасету: {df_train.shape}")

    target_col = "Score"
    feature_cols = [c for c in df_train.columns if c not in ["anime_id", "Name", target_col]]
