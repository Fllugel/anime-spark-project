...
def print_menu() -> None:
    ...
    print("")
    print("2) Підготувати ML датасети (потребує: тільки сирі дані)")
    print("   └─ Створює датасети для регресії та класифікації (без зірчастої схеми)")
    print("")
    print("3) Регресія ML (потребує: сирі дані → ML датасет регресії)")
    print("   └─ Підготовка датасету (якщо потрібно) + запуск моделі")
    print("")
    print("4) Класифікація ML (потребує: сирі дані → ML датасет класифікації)")
    print("   └─ Підготовка датасету (якщо потрібно) + запуск моделі")
    print("")
    print("0) Вихід")
    ...
    try:
        while True:
            ...
            if choice == "1":
                ...
                run_prepare_all_ml_datasets(spark, data_path=DATA_PATH)
            elif choice == "3":
