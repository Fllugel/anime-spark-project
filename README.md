...
- `oskar_q1.csv`, `oskar_q2.csv`, ... (для питань від Oskar)
- `yourname_q1.csv`, `yourname_q2.csv`, ... (для ваших питань)

#### Бізнес-питання від Oskar

Проект містить 6 додаткових бізнес-питань від Oskar, які демонструють різні типи аналізу даних:

1. **Питання 1 (Filters)**: Знайти користувачів, чия середня оцінка (`User_Mean_Score`) нижча за 6, але які при цьому подивилися (`User_Total_Completed`) більше 50 тайтлів. Це питання використовує фільтрацію для виявлення користувачів з низькою середньою оцінкою, але високою активністю перегляду.

2. **Питання 2 (JOIN)**: Вивести список аніме та оцінок, які поставив користувач 'Slayer' (`Dim_User.Username`), але лише для тих аніме, де цей користувач є "критиком" (`Fact.Rating_Deviation < 0`). Демонструє використання JOIN для об'єднання таблиць фактів та вимірів з додатковою фільтрацією.

3. **Питання 3 (GROUP BY)**: Яка середня кількість епізодів (`AVG(Dim_Anime.Episodes)`) для аніме, згрупованих за віковим рейтингом (`Dim_Anime.Age_Rating`)? Аналізує структуру аніме за віковими категоріями.

4. **Питання 4 (GROUP BY)**: Яка середня різниця (`AVG(Fact.Rating_Deviation)`) між оцінкою користувача та середньою оцінкою аніме для кожної студії (`Dim_Anime.Studios`)? Допомагає визначити, які студії отримують більш критичні або більш позитивні оцінки від користувачів.

5. **Питання 5 (GROUP BY)**: Скільки всього оцінок (`COUNT(Fact.Rating_Count)`) поставили користувачі, згруповані за статтю (`Dim_User.Gender`)? Аналізує активність користувачів за гендерною ознакою.

6. **Питання 6 (Window Functions)**: Розділити всіх користувачів на 5 груп (квінтилі) (`NTILE(5)`) на основі кількості переглянутих ними аніме (`Dim_User.User_Total_Completed`), щоб знайти "хардкорних" глядачів. Використовує віконні функції для сегментації користувачів за рівнем активності.

Всі ці питання автоматично виконуються при запуску `main.py` та зберігають результати у CSV файли.
